...
class DataCollatorForNormSeq2Seq:
    ...
    def bias_phrases_extractor(self, features, max_bias_per_sample=3):
        ...
        for sample in features:
            words = []
            for idx, (src_word_len, tgt_word_len) in enumerate(
                zip(sample["inputs_length"], sample["outputs_length"])
            ):
                src_start_idx = sum(sample["inputs_length"][:idx])
                tgt_start_idx = sum(sample["outputs_length"][:idx])
                word_input = self.tokenizer.decode(
                    sample["input_ids"][src_start_idx : src_start_idx + src_word_len]
                )
                word_output = self.tokenizer.decode(
                    sample["outputs"][tgt_start_idx : tgt_start_idx + tgt_word_len]
                )
        ...
        remain_phrase = max(
            0, max_bias_per_sample * len(features) - len(phrase_candidate)
        )
        if remain_phrase > 0:
            words_candidate = list(
                set([item for sublist in sample_output_words for item in sublist])
                - set(phrase_candidate)
            )
            ...
            for w_idx, w in enumerate(sample_output_words[i]):
                try:
                    sample_bias_lables.extend(
                        [phrase_candidate.index(w) + 1]
                        * features[i]["outputs_length"][w_idx]
                    )
                except:
                    # random ignore 0 label
                    if random.random() < 0.5:
                        sample_bias_lables.extend(
                            [0] * features[i]["outputs_length"][w_idx]
                        )
                    else:
                        sample_bias_lables.extend(
        ...
        phrase_candidate_ids = [
            self.tokenizer.encode(self.tokenizer.sp_model.DecodePieces(item.split()))
            for item in phrase_candidate
        ]
        phrase_candidate_mask = [
            [self.tokenizer.pad_token_id] * len(item) for item in phrase_candidate_ids
        ]
    ...
    def __call__(self, features, return_tensors=None):
        (
            ...
            phrase_candidate_mask,
            samples_bias_labels,
        ) = self.bias_phrases_extractor(features)

        if return_tensors is None:
            return_tensors = self.return_tensors
        labels = (
            [feature["outputs"] for feature in features]
            if "outputs" in features[0].keys()
            else None
        )
        ...
        if labels is not None:
            ...
            for feature, bias_labels in zip(features, samples_bias_labels):
                remainder = [self.label_pad_token_id] * (
                    max_label_length - len(feature["outputs"])
                )

                feature["labels"] = (
                    feature["outputs"] + [self.tokenizer.eos_token_id] + remainder
                    if padding_side == "right"
                    else remainder + feature["outputs"] + [self.tokenizer.eos_token_id]
                )
                feature["labels_bias"] = (
                    bias_labels + [0] + remainder
                    if padding_side == "right"
                    else remainder + bias_labels + [0]
                )
        ...
        outputs_bias = self.tokenizer.pad(
            {"input_ids": [feature["labels_bias"] for feature in features]},
            return_tensors=return_tensors,
        )["input_ids"]
...
def init_data():
    ...
    if not (os.path.exists(train_dataset_path) and os.path.exists(test_dataset_path)):
        ...
        with open(train_dataset_path, "w", encoding="utf-8") as file:
            for item in tqdm(train_data):
                file.write(
                    "{}\n".format(
                        json.dumps({"src": item[0], "tgt": item[1]}, ensure_ascii=False)
                    )
                )
    ...
    dataset_oov = datasets.load_dataset(
        "json", data_files={"train": train_dataset_path, "test": test_dataset_path}
    )
    print(dataset_oov)
    return dataset_oov
...
#         cache_file_names={
#             "train": "./cache/train_datasets.arrow",
#             "test": "./cache/test_datasets.arrow",
#         },
#     )
