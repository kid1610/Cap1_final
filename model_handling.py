...
import random
from model_config_handling import (
    EncoderDecoderSpokenNormConfig,
    DecoderSpokenNormConfig,
    PretrainedConfig,
)

cache_dir = "./model_spoken"
model_name = "./model_spoken"
...
class EncoderDecoderSpokenNorm(EncoderDecoderModel):
        ...
        **kwargs,
        ...
        if decoder is None:
            ...
            if "config" not in kwargs_decoder:
                decoder_config = DecoderSpokenNormConfig.from_pretrained(
            ...
            decoder = DecoderSpokenNorm.from_pretrained(
                decoder_pretrained_model_name_or_path, **kwargs_decoder
            )

        # instantiate config with corresponding kwargs
        config = EncoderDecoderSpokenNormConfig.from_encoder_decoder_configs(
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(
        ...
        if "encoder_outputs" not in model_kwargs:
            ...
            encoder_outputs, encoder_bias_outputs = encoder(
                input_ids, return_dict=True, **encoder_kwargs
            )
            model_kwargs["encoder_outputs"] = encoder_outputs
            model_kwargs["encoder_bias_outputs"] = encoder_bias_outputs
        return model_kwargs
        ...
        **kwargs,
        ...
        if self.decoder.config.add_cross_attention:
            ...
            if debug_cross_attention.is_debug:
                debug_cross_attention.add_cross_attention(
                    decoder_input_ids, decoder_outputs
                )
                debug_cross_attention.add_bias_attention_values(
                    decoder_outputs.bias_indicate_output
                )
...
class DecoderSpokenNorm(RobertaForCausalLM):
    ...
    def forward_bias_attention(self, query, values, values_mask):
        ...
        result_attention, attention_score = self.bias_attention_layer(
            query=query, key=values, value=values, mask=values_mask.bool()
        )
        result_attention = result_attention.squeeze(1).view(
            batch, output_steps, hidden_state
        )
    ...
    def forward(
        ...
        if encoder_bias_pooling is not None:
            ...
            # Make bias features
            encoder_bias_pooling = torch.cat(
                [self.mem_no_entry, encoder_bias_pooling], dim=0
            )
            mem_no_entry_feature = torch.zeros_like(
                encoder_bias_hidden_states[0]
            ).unsqueeze(0)
            mem_no_entry_mask = torch.ones_like(bias_attention_mask[0]).unsqueeze(0)
            encoder_bias_hidden_states = torch.cat(
                [mem_no_entry_feature, encoder_bias_hidden_states], dim=0
            )
            bias_attention_mask = torch.cat(
                [mem_no_entry_mask, bias_attention_mask], dim=0
            )

            # Compute ranking score
            b, s, h = sequence_output.size()
            bias_ranking_score = sequence_output.view(b * s, h).mm(
                encoder_bias_pooling.T
            )
            bias_ranking_score = bias_ranking_score.view(
                b, s, encoder_bias_pooling.size(0)
            )
            ...
            if self.training:
                ...
                if random.random() < 0.5:
                    sequence_output = sequence_output * (
                        1 - mask_indicate_output.unsqueeze(-1).repeat(1, 1, h)
                    )

            # Bias encoder hidden state
            _, max_len, _ = encoder_bias_hidden_states.size()
            bias_encoder_hidden_states = torch.index_select(
                input=encoder_bias_hidden_states,
                dim=0,
                index=bias_indicate_output.view(b * s),
            ).view(b, s, max_len, h)
            bias_encoder_attention_mask = torch.index_select(
                input=bias_attention_mask, dim=0, index=bias_indicate_output.view(b * s)
            ).view(b, s, max_len)

            sequence_atten_bias_output = self.forward_bias_attention(
                sequence_output, bias_encoder_hidden_states, bias_encoder_attention_mask
            )

            # Find output words
            prediction_scores = self.lm_head(
                sequence_output + sequence_atten_bias_output
            )
        else:
        ...
        if labels_bias is not None:
            loss_fct = CrossEntropyLoss()
            bias_ranking_loss = loss_fct(
                bias_ranking_score.view(-1, encoder_bias_pooling.size(0)),
                labels_bias.view(-1),
            )
        ...
        if not return_dict:
            ...
            return (
                ((bias_ranking_loss,) + output)
                if bias_ranking_loss is not None
                else output
            )
...
def init_tokenizer():
    ...
    tokenizer = (
        SourceFileLoader(
            "envibert.tokenizer", os.path.join(cache_dir, "envibert_tokenizer.py")
        )
        .load_module()
        .RobertaTokenizer(cache_dir)
    )
...
def init_model():
    ...
    roberta_shared = EncoderDecoderSpokenNorm.from_encoder_decoder_pretrained(
        model_name, model_name, tie_encoder_decoder=False
    )
